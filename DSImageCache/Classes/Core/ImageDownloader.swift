...
public enum DSImageCacheError: Int {
    ...
    /// code error received, you could check the value under `DSImageCacheErrorStatusCodeKey` 
...
public let DSImageCacheErrorStatusCodeKey = "statusCode"
...
open class ImageDownloader {
    ...
    class ImageFetchLoad {
        var contents = [(callback: CallbackPair, options: DSImageCacheOptionsInfo)]()
    ...
    */
    public init(name: String) {
        if name.isEmpty {
            fatalError("[DSImageCache] You should specify a name for the downloader. A downloader with empty name is not permitted.")
        }
        
        barrierQueue = DispatchQueue(label: "com.company.DSImageCache.ImageDownloader.Barrier.\(name)", attributes: .concurrent)
        processQueue = DispatchQueue(label: "com.company.DSImageCache.ImageDownloader.Process.\(name)", attributes: .concurrent)
    ...
    /**
     ...
     - parameter options:           The options could control download behavior. See `DSImageCacheOptionsInfo`.
...
extension ImageDownloader {
              ...
              completionHandler: ImageDownloaderCompletionHandler?) -> RetrieveImageDownloadTask?
    {
        if let retrieveImageTask = retrieveImageTask, retrieveImageTask.cancelledBeforeDownloadStarting {
            completionHandler?(nil, NSError(domain: DSImageCacheErrorDomain, code: DSImageCacheError.downloadCancelledBeforeStarting.rawValue, userInfo: nil), nil, nil)
            return nil
        }
        ...
        guard let url = request.url, !url.absoluteString.isEmpty else {
            completionHandler?(nil, NSError(domain: DSImageCacheErrorDomain, code: DSImageCacheError.invalidURL.rawValue, userInfo: nil), nil, nil)
            return nil
        }
    ...
    func setup(progressBlock: ImageDownloaderProgressBlock?, with completionHandler: ImageDownloaderCompletionHandler?, for url: URL, options: DSImageCacheOptionsInfo?, started: ((URLSession, ImageFetchLoad) -> Void)) {
        ...
        barrierQueue.sync(flags: .barrier) {
            ...
            loadObjectForURL.contents.append((callbackPair, options ?? DSImageCacheEmptyOptionsInfo))
...
class ImageDownloaderSessionHandler: NSObject, URLSessionDataDelegate, AuthenticationChallengeResponsable {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        {
            let error = NSError(domain: DSImageCacheErrorDomain,
                                code: DSImageCacheError.invalidStatusCode.rawValue,
                                userInfo: [DSImageCacheErrorStatusCodeKey: statusCode, NSLocalizedDescriptionKey: HTTPURLResponse.localizedString(forStatusCode: statusCode)])
            ...
            for content in fetchLoad.contents {
                ...
                if let image = image {
                    ...
                    if options.backgroundDecode {
                        let decodedImage = image.ds.decoded(scale: options.scaleFactor)
                    ...
                    if let res = task.response as? HTTPURLResponse , res.statusCode == 304 {
                        let notModified = NSError(domain: DSImageCacheErrorDomain, code: DSImageCacheError.notModified.rawValue, userInfo: nil)
                        completionHandler?(nil, notModified, url, nil)
                        continue
                    }
                    
                    let badData = NSError(domain: DSImageCacheErrorDomain, code: DSImageCacheError.badData.rawValue, userInfo: nil)
