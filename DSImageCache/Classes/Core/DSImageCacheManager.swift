...
public class RetrieveImageTask {
    ...
    /// The disk retrieve task in this image task. DSImageCache will try to look up in cache first. This task represent the cache search task.
...
/// Error domain of DSImageCache
public let DSImageCacheErrorDomain = "com.company.DSImageCache.Error"

/// Main manager class of DSImageCache. It connects DSImageCache downloader and cache.
/// You can use this class to retrieve an image via a specified URL from web or cache.
public class DSImageCacheManager {
    
    /// Shared manager used by the extensions across DSImageCache.
    public static let shared = DSImageCacheManager()
    ...
    If DSImageCacheOptions.None is used as `options`, DSImageCache will seek the image in memory and disk first.
    If not found, it will download the image at `resource.downloadURL` and cache it with `resource.cacheKey`.
    These default behaviors could be adjusted by passing different options. See `DSImageCacheOptions` for more.
    ...
    */
    ...
    func downloadAndCacheImage(with url: URL,
                      ...
                      retrieveImageTask: RetrieveImageTask,
                          progressBlock: DownloadProgressBlock?,
                      completionHandler: CompletionHandler?,
                                options: DSImageCacheOptionsInfo?) -> RetrieveImageDownloadTask?
    {
        ...
        return downloader.downloadImage(with: url, retrieveImageTask: retrieveImageTask, options: options,
            ...
            completionHandler: { image, error, imageURL, originalData in
                ...
                if let error = error, error.code == DSImageCacheError.notModified.rawValue {
        ...
        targetCache.retrieveImage(forKey: key, options: options,
            completionHandler: { image, cacheType in
                if image != nil {
                    ...
                    let error = NSError(domain: DSImageCacheErrorDomain, code: DSImageCacheError.notCached.rawValue, userInfo: nil)
