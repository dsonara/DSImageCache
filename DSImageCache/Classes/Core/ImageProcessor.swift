...
public protocol ImageProcessor {
    ...
    func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image?
}

typealias ProcessorImp = ((ImageProcessItem, DSImageCacheOptionsInfo) -> Image?)
...
public struct RoundCornerImageProcessor: ImageProcessor {
    ...
    public init(cornerRadius: CGFloat, targetSize: CGSize? = nil) {
        ...
        if let size = targetSize {
            self.identifier = "com.company.DSImageCache.RoundCornerImageProcessor(\(cornerRadius)_\(size))"
        } else {
            self.identifier = "com.company.DSImageCache.RoundCornerImageProcessor(\(cornerRadius))"
        }
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            let size = targetSize ?? image.ds.size
            return image.ds.image(withRoundRadius: cornerRadius, fit: size)
        case .data(_):
...
public struct ResizingImageProcessor: ImageProcessor {
    ...
    public init(targetSize: CGSize, contentMode: ContentMode = .none) {
        ...
        if contentMode == .none {
            self.identifier = "com.company.DSImageCache.ResizingImageProcessor(\(targetSize))"
        } else {
            self.identifier = "com.company.DSImageCache.ResizingImageProcessor(\(targetSize), \(contentMode))"
        }
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            return image.ds.resize(to: targetSize, for: targetContentMode)
        case .data(_):
            return (DefaultImageProcessor.default >> self).process(item: item, options: options)
        }
    }
...
public struct BlurImageProcessor: ImageProcessor {
    ...
    public init(blurRadius: CGFloat) {
        ...
        self.identifier = "com.company.DSImageCache.BlurImageProcessor(\(blurRadius))"
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        ...
        case .image(let image):
            ...
            return image.ds.blurred(withRadius: radius)
        case .data(_):
            return (DefaultImageProcessor.default >> self).process(item: item, options: options)
        }
    }
...
public struct OverlayImageProcessor: ImageProcessor {
    ...
    public init(overlay: Color, fraction: CGFloat = 0.5) {
        ...
        self.identifier = "com.company.DSImageCache.OverlayImageProcessor(\(overlay.hex)_\(fraction))"
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            return image.ds.overlaying(with: overlay, fraction: fraction)
        case .data(_):
            return (DefaultImageProcessor.default >> self).process(item: item, options: options)
        }
    }
...
public struct TintImageProcessor: ImageProcessor {
    ...
    public init(tint: Color) {
        self.tint = tint
        self.identifier = "com.company.DSImageCache.TintImageProcessor(\(tint.hex))"
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            return image.ds.tinted(with: tint)
        case .data(_):
            return (DefaultImageProcessor.default >> self).process(item: item, options: options)
        }
    }
...
public struct ColorControlsProcessor: ImageProcessor {
    ...
    public init(brightness: CGFloat, contrast: CGFloat, saturation: CGFloat, inputEV: CGFloat) {
        ...
        self.identifier = "com.company.DSImageCache.ColorControlsProcessor(\(brightness)_\(contrast)_\(saturation)_\(inputEV))"
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            return image.ds.adjusted(brightness: brightness, contrast: contrast, saturation: saturation, inputEV: inputEV)
        case .data(_):
            return (DefaultImageProcessor.default >> self).process(item: item, options: options)
        }
    }
...
public struct BlackWhiteProcessor: ImageProcessor {
    ...
    /// Identifier of the processor.
    /// - Note: See documentation of `ImageProcessor` protocol for more.
    public let identifier = "com.company.DSImageCache.BlackWhiteProcessor"
...
public struct CroppingImageProcessor: ImageProcessor {
    ...
    public init(size: CGSize, anchor: CGPoint = CGPoint(x: 0.5, y: 0.5)) {
        self.size = size
        self.anchor = anchor
        self.identifier = "com.company.DSImageCache.CroppingImageProcessor(\(size)_\(anchor))"
    }
    ...
    public func process(item: ImageProcessItem, options: DSImageCacheOptionsInfo) -> Image? {
        switch item {
        case .image(let image):
            return image.ds.crop(to: size, anchorOn: anchor)
